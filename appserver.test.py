...
requestTopic = "requests_test"
responseTopic = "responses_test"

consumer = KafkaConsumer(bootstrap_servers=[kafkaServer],auto_offset_reset='latest')
consumer.subscribe([responseTopic])

class KafkaProducerThread(object):
    def __init__(self, method, action, data):
        self.interval = 1
        thread = threading.Thread(target=self.run, args=(method, action, data))
        thread.daemon = True
        thread.start()
    def run(self, method, action, data):
        actionList = [
            'guild_analysis',
            '3dclustering',
        ...
        for message in consumer:
            ...
            break
        return str(ret['data'])
        #t = KafkaConcumerThread(request_id)
        #return t
############################################################################################

class KafkaConcumerThread(object):
    def __init__(self, request_id):
        self.interval = 1
        thread = threading.Thread(target=self.run, args=(request_id, ))
        thread.daemon = True
        thread.start()
    def run(self, request_id):

        print("consumer of request id '" + str(request_id) + "' starting ...")
        return Response("resp2", status=200, mimetype='text/plain')
        consumer = KafkaConsumer(bootstrap_servers=[kafkaServer], auto_offset_reset='latest')
        consumer.subscribe([responseTopic])
        #global cunsumer
        ret = ''
...
def guild_analysis():
    print("new request")
    def runT():
        t = KafkaProducerThread(request.method, 'guild_analysis','data1')
        return "salam"
    return Response(runT(), status=200, mimetype='text/plain')
